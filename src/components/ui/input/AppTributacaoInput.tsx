...
type Props = {
  ...
  /**
   * IMPORTANTE:
   * quando carregar outra empresa/tela e quiser que o display resete,
...
export function AppTributacaoInput({
  ...
  // Inicializacao do display SEM useEffect.
  ...
  }, [syncKey]); // so muda quando syncKey muda
  ...
  const [percentualFatorRDisplay, setPercentualFatorRDisplay] =
    useState<string>(initialFatorRDisplay);
  ...
  return (
    <div className="flex w-full flex-wrap gap-4" key={syncKey}>
      <AppSelectInput
      ...
      {showSimplesFields && (
        <>
          <AppTextInput
            ...
            required
            title="RBT12"
            tooltip="RBT12 = Receita Bruta Total acumulada dos ultimos 12 meses."
            placeholder=""
          />
